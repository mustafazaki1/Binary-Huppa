...
void * create_user_page_WS(unsigned int numOfElements)
{
	...
	uint32 start = (uint32)kmalloc((numOfElements)*sizeof(struct WorkingSetElement)); // allocating memory for the WS
	return (void*)start;
}
...
void * create_user_directory()
{
	...
	uint32 *ptr_dir = (uint32 *)kmalloc(PAGE_SIZE); // allocate a page for user dir
	if(ptr_dir==NULL)// if no memory return NULL
		return NULL;
	return (void*)ptr_dir;
}
