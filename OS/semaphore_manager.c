...
//	panic("createSemaphore() is not implemented yet...!!");
	...
	int FindSemaphore=get_semaphore_object_ID(ownerEnvID,semaphoreName);
	if (FindSemaphore!=E_SEMAPHORE_NOT_EXISTS)
		return E_SEMAPHORE_EXISTS;
	else
	{
		struct Semaphore* NewSemaphore=NULL;
		int AllocateSemaphore=allocate_semaphore_object(&NewSemaphore);
		if (AllocateSemaphore==E_NO_SEMAPHORE)
			return AllocateSemaphore;
		else
		{
			strcpy(NewSemaphore->name,semaphoreName);
			NewSemaphore->ownerID=ownerEnvID;
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int SemaphoreIndex=get_semaphore_object_ID(ownerEnvID,semaphoreName);
	if (SemaphoreIndex!=E_SEMAPHORE_NOT_EXISTS)
	{
		struct Semaphore TheSemaphore=semaphores[SemaphoreIndex];
	//	cprintf("before if :%d\n",TheSemaphore.value);
		semaphores[SemaphoreIndex].value--;
		if(semaphores[SemaphoreIndex].value<0)
		{
			//cprintf("inside if :%d\n",TheSemaphore.value);

			sched_remove_ready(myenv);
			enqueue(&semaphores[SemaphoreIndex].env_queue,myenv);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	int SemaphoreIndex=get_semaphore_object_ID(ownerEnvID,semaphoreName);
		if (SemaphoreIndex!=E_SEMAPHORE_NOT_EXISTS)
		{
			...
			if (semaphores[SemaphoreIndex].value<=0)
			{
				struct Env* myenv = dequeue(&semaphores[SemaphoreIndex].env_queue);
