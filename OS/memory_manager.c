...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32* table = kmalloc(PAGE_SIZE); // allocate a page for user dir
	uint32 pa = kheap_physical_address((uint32)table);
	if(table == NULL) // if no memory available
		return NULL;
	for (int i=0;i<1<<10;i++) // clear all entries
		table[i]=0;
	pa|=PERM_PRESENT | PERM_USER | PERM_WRITEABLE; // set permestions
	ptr_page_directory[PDX(virtual_address)]=pa;
	tlbflush();
	return (void*)table;
}
