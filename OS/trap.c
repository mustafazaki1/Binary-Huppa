...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	if (WorkingSetCurrentSize >= curenv->page_WS_max_size)
	{
		...
		//that check if that page was modified or not
		uint32 modified = pt_get_page_permissions(curenv, VictimVA);
		modified &= PERM_MODIFIED;
		if (modified != 0)
		{
			//if it was modified then update the page file
			int ret = pf_update_env_page(curenv, (void*) VictimVA, frame_info);
	...
	uint32* EnvPageDirectory = curenv->env_page_directory;
	map_frame(EnvPageDirectory, frame_info, (void*) fault_va,PERM_USER | PERM_WRITEABLE);
	uint32 page_data = pf_read_env_page(curenv, (void*) fault_va);
	if (page_data == E_PAGE_NOT_EXIST_IN_PF)
	{
		//if the page in not found in the page file check if it is a stack page
		if (fault_va >= USER_HEAP_MAX && fault_va < USTACKTOP)
		{
			//if it is a stack page add it to the page file
