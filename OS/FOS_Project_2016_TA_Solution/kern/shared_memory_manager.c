...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	struct Share *retshare;
	int ret =allocate_share_object(&retshare);
	if(ret==E_NO_SHARE)
	{
		return E_NO_SHARE;
	}
	else
	{
		for(int i=0;i<size;i++)
		{
			...
			if (frame_info!=NULL)
			{
				map_frame(curenv->env_page_directory,frame_info,(void*)virtual_address,PERM_WRITEABLE|PERM_USER);
				virtual_address+=PAGE_SIZE;
				add_frame_to_storage(retshare->framesStorage,frame_info,i);
			}
		}
		//Step 3 Not Implemented
		shares[ret].ownerID=ownerID;
		strcpy(shares[ret].name,shareName);
		shares[ret].size=size;
		shares[ret].references=1;
		shares[ret].isWritable=isWritable;
	}
...
int getSizeOfSharedObject(int32 ownerID, char* shareName)
{
	...
	if (SharedObjectIndex==E_SHARED_MEM_NOT_EXISTS)
		...
		return shares[SharedObjectIndex].size;
...
//	panic("getSharedObject() is not implemented yet...!!");
	...
	int FramesNum= shares[SharedObjectIndex].size;
	virtual_address=ROUNDDOWN(virtual_address,PAGE_SIZE);
	for(int i=0;i<FramesNum;i++)
	{
		struct Frame_Info* frame_info = get_frame_from_storage(shares[SharedObjectIndex].framesStorage,i);
		if(frame_info==NULL)
		{
			return E_SHARED_MEM_NOT_EXISTS;
		}
	...
	int temp=0;
	if(shares[SharedObjectIndex].isWritable==1)
		temp=PERM_WRITEABLE;
	for(int i=0;i<FramesNum;i++)
	{
		struct Frame_Info* frame_info = get_frame_from_storage(shares[SharedObjectIndex].framesStorage,i);
	//	3) Share these frames with the current environment "myenv" starting from the given "virtual_address"
		map_frame(curenv->env_page_directory,frame_info,(void*)virtual_address,PERM_USER|temp);
		virtual_address+=PAGE_SIZE;
